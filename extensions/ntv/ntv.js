...
const extension = {
  ...
  fetchContentMetadata: async (url) => {
    const pageResponse = await http.fetch(url);
    ...
    const xmlResponse = await http.fetch(`https://www.ntv.ru/vi${videoId}/`);
    ...
    logger.info(`Video: ${hqFileLink}`);
    if (subtitlesRoute) logger.info(`Subtitles: ${subtitlesUrl}`);
